...
class Casestudy extends React.Component {
  ...
  cleanData = () => {
    ...
    if (this.state.doc[0].data.light_dark_mode) {
      this.setState({
  ...
  //handles that 'close' of the icon i.e. back to homepage
  handleGoBack = () => {
    history.back();
  };
    ...
    Prismic.api(apiEndpoint, { accessToken: token }).then(api => {
      api
        .query(Prismic.Predicates.at("document.type", "casestudy"), {
          after: `${this.props.id}`,
          fetch: ["casestudy.slugs", "casestudy.id"]
        })
        .then(response => {
          if (response) {
            this.setState({
              nextCasestudyId: response.results[0].id,
              nextCasestudySlug: response.results[0].slugs[0]
            });
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <h1 onClick={this.handleGoBack}>casestudy!!!</h1>
