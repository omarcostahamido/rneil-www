import React from 'react'
import Nav from './Nav'
import Header from './Header'
import Casestudy_Featured from './Casestudy_Featured'


/*
do I call for data at this level???? 
yes and pass it down to the children?? 
yes - api calls here for the homepage logo
for the case studies - check how many and get hero/title
maybe I should call for header data in its own component?? 
 */ 


// const api = {
//   url: process.env.REACT_APP_API_URL,
//   spaceId: process.env.REACT_APP_SPACE_ID,
//   accessToken: process.env.REACT_APP_ACCESS_TOKEN,
//   environment: "master",
//   getAllEntries: `${this.url}/spaces/${this.spaceID}/environments/${this.environment}/entries?access_token=${this.accessToken}`
// }


class Homepage extends React.Component {

  state = {
    headerMainCopy: 'how terrible and how beautiful'
  }

  //maybe not necessary??? need to read more about babel-'transform-class-properties'
  // constructor(props) {
  //   super(props)
  // }
  getContentfulData = () => {
    const { token, spaceId, baseUrl } = this.props
    const url = `${baseUrl}/spaces/${spaceId}/environments/master/entries?access_token=${token}`

    return fetch(url)
      .then(response => response.json())
      .then(data => (data))
  }

  //first...just get the header. don't worry about sorting for now
  //once you start thinking about sorting...you'll need to make a content
  //object that has casestudies and whatever other content types as objects within?? 

  cleanContentfulData = (data) => {
    return data.items.map(item => {
      return item.fields
    })
  }

  //right now this just returns the fields array and we need more data
  //than that to check the ids 

  //LIFECYCLE----------------------------------
  componentDidMount() {
    //api call here. let's start with something simple like the header copy
   this.getContentfulData()
    .then(data => this.cleanContentfulData(data))
    .then(cleanData => {
      this.setState({
        headerMainCopy: cleanData[1].headerMainCopy
      })
    })
  }

  render() {
    return (
      <div>
        <Nav 
          logo="data:image/jpeg;base64,"
        />
        <Header 
          copy={this.state.headerMainCopy}
          img="https://images.unsplash.com/photo-1547922657-b370d1687eb1?ixlib=rb-1.2.1&auto=format&fit=crop&w=1600&q=80"
        />
        <Casestudy_Featured
          title="Casestudy 1"
          hero="https://images.unsplash.com/photo-1547898812-9e25c5a693e0?ixlib=rb-1.2.1&auto=format&fit=crop&w=2168&q=80"
        />
        <Casestudy_Featured
          title="Casestudy 2"
          hero="https://images.unsplash.com/photo-1547958600-915c8a5131de?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2102&q=80"
        />
      </div>
    )
  }
}

export default Homepage