 ...
 * const handleImageClick = () => {
    ...
    return function handleScroll(e) {
      if (window.innerWidth > 1024) {
        images = [].slice.call(e.target.parentNode.children);
        if(!prevImage) {
          prevImage = images.filter(image => image === e.target);
        }
        currentImage = images.filter(image => image === e.target);
        ...
        if (
          ...
          if (/^slice/.test(e.target.classList[0])) {
            if (eventX < e.pageX) {
              scrollX += e.target.getBoundingClientRect().width * 0.25;
            } else {
              scrollX -= e.target.getBoundingClientRect().width * 0.25;
            }
          } else {
